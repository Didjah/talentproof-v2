    ...
    <section className="bg-[#1B3A6B] py-16 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {METIERS.map((m) => (
            <div key={m.titre} className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-5 hover:bg-white/15 transition-all">
              {/* Header */}
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`text-white text-xs font-semibold px-2 py-1 rounded-full ${m.tensionColor}`}>
                  {m.tension}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-1 mb-3">
                {m.pays.map((p) => (
                  <span key={p} className="text-white/80 text-xs bg-white/10 px-2 py-0.5 rounded-full">{p}</span>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-1 mb-3">
                {m.competences.map((c) => (
                  <span key={c} className="text-[#C9A84C]/80 text-xs border border-[#C9A84C]/30 px-2 py-0.5 rounded-full">{c}</span>
                ))}
              </div>
