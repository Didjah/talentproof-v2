...
export default function TalentCardMedia({
  ...
  return (
    ...
    <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 w-full">
      ...
      {/* ── 4. BOUTONS CONTACT ── */}
      <div className="px-4 py-4 border-t border-gray-100 grid grid-cols-3 gap-2">
        {telephone && (
          <a href={`https://wa.me/${telephone.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer"
            className="flex items-center justify-center gap-1.5 bg-green-500 hover:bg-green-600 text-white text-xs sm:text-sm font-semibold py-2.5 rounded-xl transition-colors">
        ...
        {telephone && (
          <a href={`tel:${telephone}`}
            className="flex items-center justify-center gap-1.5 bg-[#1B3A6B] hover:bg-[#142d54] text-white text-xs sm:text-sm font-semibold py-2.5 rounded-xl transition-colors">
        ...
        <Link href={`/profil/${id}`}
          ...
          <span className="hidden sm:inline">Voir </span>profil →
        </Link>
      </div>
    </div>
      {lightbox && (
